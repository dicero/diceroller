...
var Register = new function() {
  var registerUrl = "hello", //注册;
      getCaptchaUrl = "",    //获取图片验证码;
      getMobileCodeUrl = ""; //获取手机验证码;

  //获取用户注册信息;
  this._getUserInfo = function() {
    var pData = turnToJsonData($("#userInfoR").serializeArray());
  ...
  this._init = function(userInfo) {
    ...
    $("#userInfoF").validate({
      ...
      rules: {
        ...
        password: {
          required: true,
          customRegExp: /^(?![a-zA-Z]+$)(?!\d+$)[a-zA-Z0-9]{9,22}$/
        },
        passwordAgain: {
          required: true,
          equalTo: "#uPassword"
      ...
      messages: {
        ...
        password: {
          required: "密码不能为空",
          customRegExp: "密码由9-22位字母和数字组成，字母区分大小写"
        },
        passwordAgain: {
          required: "请再次输入密码",
          equalTo: "两次密码输入不一致"
  ...
  this._register = function() {
    ...
    //向后台发送注册请求;
    // Ajax._post(registerUrl, pData).success(function(data) {
